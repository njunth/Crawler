...
class ProxyMiddleware(object):
    ...
    def __init__(self, ip=''):
        self.ip = ip
        self.mongodbname = "Crawler"
        self.collection = "ip_mongodb_name"
    # def __init__(self):
    ...
    def process_request(self, request, spider):
        ...
        try:
            res = collection.find_one( {'_id': "ip"} )
            if res != None:
                request.meta['proxy'] = 'http://' + str(res['value'])
                print request.meta['proxy']
            else:
                print "mongod search fail"
        except:
            # request.meta['proxy'] = 'http://127.0.0.1:9743'
            print "mongod link fail"
